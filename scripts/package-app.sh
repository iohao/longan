...
LOCAL_UPDATER_KEY_PATH="$ROOT_DIR/_local/tauri-updater.key"
SKIP_INSTALL=0
SKIP_TYPECHECK=0
OPEN_RESULT=0
...
artifact into a stable output directory using Bun.
...
Options:
  ...
  --skip-typecheck      Skip typecheck/build check before packaging
...
if [[ "$SKIP_TYPECHECK" -eq 0 ]]; then
  log "Running build check (tsc & vite build)"
...
log "Packaging desktop app with Tauri via Bun"
...
(
  ...
  if [[ -x "$TAURI_CLI" ]]; then
    ...
    bunx tauri build --bundles "$BUNDLE_TARGET"
  fi
)
