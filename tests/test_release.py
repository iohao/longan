...
class ReleaseStateTest(unittest.TestCase):
    ...
    def test_locked_notes_cannot_change(self) -> None:
        ...
        self.notes.write_text("# Longan v0.1.0\nchanged\n", encoding="utf-8")
    ...
    def test_existing_tag_must_match_head_and_notes(self) -> None:
        with mock.patch.object(release, "tag_exists", return_value=True), mock.patch.object(
            ...
            release.validate_existing_annotated_tag(self.root, "v0.1.0", "abc123", "reviewed notes")
        with mock.patch.object(release, "tag_exists", return_value=True), mock.patch.object(
    ...
    def test_create_tag_preserves_markdown_heading(self) -> None:
        reviewed_notes = self.notes.read_text(encoding="utf-8")
        with mock.patch.object(release, "tag_exists", side_effect=[False, False]), mock.patch.object(
            release, "run"
        ) as run, mock.patch.object(release, "validate_remote_tag"):
    ...
    def test_preflight_does_not_query_github_actions(self) -> None:
        with mock.patch.object(release, "require_commands"), mock.patch.object(
            ...
            release.notes, "repo_from_origin", return_value="iohao/longan"
        ), mock.patch.object(
            release, "assert_github_access"
        ), mock.patch.object(
            release.notes, "read_versions", return_value="0.1.0"
        ), mock.patch.object(
            release, "tag_exists", return_value=False
        ), mock.patch.object(
    ...
    def test_github_access_requires_public_repo_and_signing_secrets(self) -> None:
        auth = CompletedProcess(["gh"], 0, "", "")
        with mock.patch.object(release, "run", return_value=auth), mock.patch.object(
            release,
            "json_output",
            side_effect=[{"nameWithOwner": "iohao/longan", "visibility": "PRIVATE"}],
        ):
            with self.assertRaisesRegex(release.ReleaseError, "must be public"):
                release.assert_github_access(self.root, "iohao/longan")

        with mock.patch.object(release, "run", return_value=auth), mock.patch.object(
            release,
            "json_output",
            side_effect=[
                {"nameWithOwner": "iohao/longan", "visibility": "PUBLIC"},
                [{"name": "TAURI_SIGNING_PRIVATE_KEY"}],
            ],
        ):
            with self.assertRaisesRegex(release.ReleaseError, "TAURI_SIGNING_PRIVATE_KEY_PASSWORD"):
                release.assert_github_access(self.root, "iohao/longan")
