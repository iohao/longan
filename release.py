#!/usr/bin/env python3
"""Prepare, publish, and recover a Longan GitHub Release."""
...
from scripts import longan_release as notes
...
def require_commands() -> None:
    ...
    if sys.version_info < (3, 11):
        raise ReleaseError("Python 3.11 or newer is required for TOML version parsing")
...
def assert_github_access(root: Path, repo: str) -> None:
    ...
    details = json_output(["gh", "repo", "view", repo, "--json", "nameWithOwner,visibility"], cwd=root)
    ...
    secrets = json_output(["gh", "secret", "list", "--repo", repo, "--json", "name"], cwd=root)
    names = {entry.get("name") for entry in secrets if isinstance(entry, dict)}
    missing = sorted(REQUIRED_SECRETS - names)
    if missing:
        raise ReleaseError(f"Repository is missing required updater signing secrets: {', '.join(missing)}")


def github_release_exists(root: Path, repo: str, tag: str) -> bool:
    result = run(["gh", "release", "view", tag, "--repo", repo], cwd=root, capture=True, check=False)
...
def set_version(root: Path, version: str) -> dict[str, str]:
    ...
    print(f"Updated Longan version to {version} in:")
    for path in updated:
        print(f"- {path}")
    return updated
...
def validate_remote_tag(root: Path, tag: str, expected_head: str) -> None:
    ...
    if actual != expected_head:
        raise ReleaseError(f"Tag {tag} must point to prepared HEAD {expected_head}, found {actual or 'missing'}")
...
def validate_existing_annotated_tag(root: Path, tag: str, expected_head: str, expected_notes: str) -> None:
    if not tag_exists(root, tag):
        return
    if git(root, "cat-file", "-t", f"refs/tags/{tag}") != "tag":
...
def create_and_push_tag(context: ReleaseContext, state: dict[str, Any], release_notes: str) -> None:
...
def show_status(root: Path, version: str | None) -> None:
    ...
    try:
        ...
        if release.returncode == 0:
            observed["release"] = json.loads(release.stdout)
        else:
            observed["release"] = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish a reviewed Longan GitHub Release")
    subparsers = parser.add_subparsers(dest="command", required=True)
    set_version_parser = subparsers.add_parser(
        "set-version", help="update every tracked Longan application version"
    )
